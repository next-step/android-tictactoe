...
class TictactoeGameTest {
    ...
    fun `X 3줄이 완성 되면 게임이 종료`() {
        val game = TictactoeGame()
        game.setPosition(true, CellPosition.TOP_LEFT)
        game.setPosition(false, CellPosition.MIDDLE_LEFT)
    ...
    fun `O 3줄이 완성 되면 게임이 종료`() {
        ...
        game.setPosition(true, CellPosition.TOP)
        game.setPosition(false, CellPosition.MIDDLE)
        game.setPosition(true, CellPosition.BOTTOM_RIGHT)
        val actual = game.setPosition(false, CellPosition.MIDDLE_RIGHT)
    ...
    fun `3줄이 완성 되지 않은 상태이면 계속 게임 진행상태`() {
        val game = TictactoeGame()
        game.setPosition(true, CellPosition.TOP_LEFT)
        val actual = game.setPosition(false, CellPosition.TOP)
    ...
    fun `모든 칸이 완성되어도 게임이 끝나지 않으면 무승부`() {
        ...
        game.setPosition(false, CellPosition.BOTTOM_RIGHT)
        val actual = game.setPosition(true, CellPosition.BOTTOM)
