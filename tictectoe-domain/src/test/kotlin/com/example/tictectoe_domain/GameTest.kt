...
class GameTest {
    ...
    fun `첫 순서에 보드의 중단 오른쪽을 선택하면 해당 위치에 Player1이 차지한다`() {
        ...
        game.selectBoard(Position.MID_RIGHT, TwoPlayer)
    ...
    fun `2인 모드에서 두번째 순서에 보드의 중단 가운데를 선택하면 해당 위치에 Player2이 차지한다`() {
        ...
        game.selectBoard(Position.MID_RIGHT, TwoPlayer)
        game.selectBoard(Position.MID_CENTER)
    ...
    fun `선택됐던 보드를 선택하면 해당 위치에 아무 변화가 없다`() {
        ...
        game.selectBoard(Position.MID_RIGHT, TwoPlayer)
        game.selectBoard(Position.MID_RIGHT)
    ...
    @Test
    fun `초기 게임 알고리즘은 랜덤모드이다`() {
        // given : 공백 보드를 가진 게임을 시작한다.
        val game = Game(Board.EMPTY, rule)

        // then : 게임 모드는 랜덤모드이다.
        assertThat(game.gameAlgorithm).isEqualTo(Random)
    }
    ...
    fun `게임모드를 랜덤 모드에서 2인 모드로 변경할 수 있다`() {
        ...
        assertThat(game.gameAlgorithm).isEqualTo(TwoPlayer)
    }
}
